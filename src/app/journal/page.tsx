...
function JournalContent() {
  ...
  // ... (EFFECTS & DATA FETCHING - KEEP AS IS)
  useEffect(() => {
    const init = async () => {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
      const val = e.target.value; setNewMessage(val);
      const cursorPos = e.target.selectionStart; const textBeforeCursor = val.slice(0, cursorPos); const match = textBeforeCursor.match(/@(\w*)$/); 
  ...
  const JournalCollage = ({ imagesStr }: { imagesStr?: string }) => {
      ...
      return (
          <div className="grid grid-cols-2 grid-rows-2 w-full h-full">
              <div className={`relative ${images.length === 1 ? 'col-span-2 row-span-2' : ''} ${images.length === 3 ? 'row-span-2' : ''} overflow-hidden border-r border-b border-black/10`}><img src={images[0]} className="w-full h-full object-cover" alt="cover" loading="lazy" /></div>
                    ...
                    <ScrollArea className="flex-1 p-3">
                        <div className="space-y-1 pb-2">
                            ...
                            {posts.map((post, index) => {
                                ...
                                // 10 minutes timeout for grouping
                                ...
                                return (
                                    <div 
                                        key={post.id} 
                                        // mt-[2px] is the key to tight grouping
                                    ...
                                    >
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            {/* Font size [13px] matches Discord */}
                                        ...
                                        {post.username === username && (
                                            <button onClick={() => handleDeletePost(post.id)} className="absolute right-2 top-1 text-white/10 hover:text-red-400 opacity-0 group-hover:opacity-100 p-1"><Trash2 className="w-3 h-3" /></button>
                                        )}
                                    </div>
                                );
                            })}
