// src/app/watch-together/[roomId]/page.tsx
'use client';
...
import type { ChatMessage, Participant, UserProfile, WatchTogetherRoomState } from '@/lib/types';
...
export default function WatchTogetherRoomPage({ params }: { params: { roomId: string } }) {
    const { roomId } = React.use(params as any);
    const { user, profile, loading: authLoading } = useAuth();
    const router = useRouter();
    const { toast } = useToast();

    const [roomState, setRoomState] = React.useState<WatchTogetherRoomState | null>(null);
    const [chatMessages, setChatMessages] = React.useState<ChatMessage[]>([]);
    
    // Firestore update function
    const updateRoomState = React.useCallback(async (newState: Partial<WatchTogetherRoomState>) => {
        if (!roomId) return;
    ...
    React.useEffect(() => {
        ...
        const roomRef = doc(db, 'watchTogetherRooms', roomId);
        const chatQuery = query(collection(db, 'watchTogetherRooms', roomId, 'chats'), orderBy('timestamp', 'asc'));

        const unsubscribeRoom = onSnapshot(roomRef, (docSnap) => {
            if (!docSnap.exists()) {
                toast({ title: "Error", description: "Watch Together room not found.", variant: "destructive" });
                router.push('/watch-together');
                return;
            }

            const roomData = docSnap.data() as WatchTogetherRoomState;
            ...
            updateUserProfile(user.uid, { status: { isStudying: false, isWatching: true, roomId: roomId } });
        });
        ...
        return () => {
            ...
            unsubscribeChat();
            if (user && profile) {
                getDoc(doc(db, 'watchTogetherRooms', roomId)).then(docSnap => {
                    if(docSnap.exists()) {
                ...
                updateUserProfile(user.uid, { status: { isStudying: false, isWatching: false, roomId: null } });
            }
        };
     ...
     const handleSendMessage = async (message: {text: string}, replyTo: { id: string, text: string } | null) => {
        ...
        const chatCollectionRef = collection(db, 'watchTogetherRooms', roomId, 'chats');
