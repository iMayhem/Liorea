...
export default function WatchTogetherPage() {
  ...
  const handleCreateRoom = async () => {
    ...
    setIsCreating(true);
    try {
      const newRoomRef = doc(collection(db, 'watchTogetherRooms'));
      ...
      router.push(`/watch-together/${newRoomRef.id}`);
    } catch (error) {
  ...
  const doJoinRoom = async (id: string) => {
    ...
    const roomRef = doc(db, 'watchTogetherRooms', id);
    const roomSnap = await getDoc(roomRef);

    if (roomSnap.exists()) {
      router.push(`/watch-together/${id}`);
    } else {
        if (id === PUBLIC_WATCH_TOGETHER_ROOM_ID) {
  ...
  const handleJoinPublicRoom = async () => {
    ...
    try {
        await doJoinRoom(PUBLIC_WATCH_TOGETHER_ROOM_ID);
    } catch (error) {
